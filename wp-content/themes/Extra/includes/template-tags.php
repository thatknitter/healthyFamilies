...
function extra_blog_feed_get_content() {
	...
	$order = sanitize_text_field( $_POST['order'] );
	$content_length = absint( $_POST['content_length'] );
	$hover_overlay_icon = sanitize_text_field( $_POST['hover_overlay_icon'] );
...
function extra_contact_form_submit() {
	...
	if ( !isset( $_POST['nonce_extra_contact_form'] ) || !wp_verify_nonce( $_POST['nonce_extra_contact_form'], 'extra-contact-form' ) ) {
		$message = array(
			'message' => __( 'Form submission error, please refresh and try again.', 'extra' ),
			'type'    => 'error',
		);
		$error = true;
	}
	...
	if ( empty( $_POST['contact_name'] ) ) {
		$message = array(
			'message' => __( 'Name field cannot be empty.', 'extra' ),
			'type'    => 'error',
		);
		$error = true;
	}
	...
	if ( empty( $_POST['contact_email'] ) ) {
		$message = array(
			'message' => __( 'Email field cannot be empty.', 'extra' ),
			'type'    => 'error',
		);
		$error = true;
	}
	...
	if ( ! is_email( $_POST['contact_email'] ) ) {
		$message = array(
			'message' => __( 'Please enter a valid email address.', 'extra' ),
			'type'    => 'error',
		);
		$error = true;
	}
	...
	if ( !$error ) {
		...
		$subject = sprintf(
			__( 'New Message From %1$s%2$s', 'extra' ),
			sanitize_text_field( get_option( 'blogname' ) ),
			( '' !== $contact_page_options['title'] ? sprintf( _x( ' - %s', 'contact form title separator', 'extra' ), sanitize_text_field( $contact_page_options['title'] ) ) : '' )
		);
		...
		$message = array(
			'message' => __( 'Thanks for contacting us.', 'extra' ),
			'type'    => 'success',
		);
	}
	...
	if ( empty( $message ) ) {
		$message = array(
			'message' => __( 'There was a problem, please try again.', 'extra' ),
			'type'    => 'error',
		);
	}
...
function extra_get_video_embed( $video_urls ) {
	...
	if ( false === strpos( $video_urls, ',' ) ) {
		$video_url = esc_url( $video_urls );
	...
	if ( ! empty( $provider ) ) {
		$video_src = $oembed->get_html( $video_url, $oembed_args );
	} else {
		...
		foreach ( $video_urls as $video_url ) {
			...
			if ( !empty( $type['type'] ) ) {
				$sources .= sprintf( '<source type="%s" src="%s" />',
					esc_attr( $type['type'] ),
					esc_attr( $video_url )
				);
			}
