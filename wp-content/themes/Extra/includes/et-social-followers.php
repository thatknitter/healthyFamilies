...
class ET_Social_Followers {
	...
	public function get_followers_text( $count ) {
		return sprintf( _n( '%s follower', '%s followers', $count, 'extra' ), $this->number_format( $count ) );
	}
...
class ET_Facebook_Social_Followers extends ET_Social_Followers {
	...
	function get_fields() {
		$fields = array(
			'name'   => array(
				'label'       => __( 'Page Name', 'extra' ),
				'type'        => 'text',
				'default'     => $this->get_default_api_data( 'client_id' ),
				'description' => __( 'The name of your facebook page, not the full URL. If your facebook url is: "http://www.facebook.com/elegantthemes", then your page name is: "elegantthemes"', 'extra' ),
			),
			'id'     => array(
				'label'               => __( 'App ID', 'extra' ),
				'type'                => 'password',
				'default'             => $this->get_default_api_data( 'id' ),
				'authorization_field' => true,
				'description'         => __( 'Enter the App ID', 'extra' ),
			),
			'secret' => array(
				'label'               => __( 'App Secret', 'extra' ),
				'type'                => 'password',
				'default'             => $this->get_default_api_data( 'secret' ),
				'authorization_field' => true,
				'description'         => __( 'Enter the App Secret', 'extra' ),
			),
		);
		return $fields;
	}
	...
	public function get_followers_text( $count ) {
		return sprintf( _n( '%s like', '%s likes', $count, 'extra' ), $this->number_format( $count ) );
	}
...
class ET_Google_Plus_Social_Followers extends ET_Social_Followers {
	...
	function get_fields() {
		$fields = array(
			'userId'  => array(
				'label'       => __( 'Google+ User Id', 'extra' ),
				'type'        => 'text',
				'default'     => $this->get_default_api_data( 'client_id' ),
				'description' => __( 'You can find the User Id at the end of your Google+ URL. For example if the URL was: http://plus.google.com/+elegentthemes, the User Id would be: +elegenthemes', 'extra' ),
			),
			'api_key' => array(
				'label'               => __( 'API Key', 'extra' ),
				'type'                => 'password',
				'authorization_field' => true,
				'default'             => $this->get_default_api_data( 'api_key' ),
				'description'         => sprintf( __( 'Google API key. You can find all the info on how to obtain one here: %s', 'extra' ), '<a target="_blank" href="https://developers.google.com/+/api/oauth#apikey">https://developers.google.com/+/api/oauth#apikey</a>' ),
			),
		);
		return $fields;
	}
	...
	public function get_followers_text( $count ) {
		return sprintf( _n( '%s +1', '%s +1s', $count, 'extra' ), $this->number_format( $count ) );
	}
...
class ET_Youtube_Social_Followers extends ET_Social_Followers {
	...
	function get_fields() {
		$fields = array(
			'username' => array(
				'label'   => __( 'Youtube Username', 'extra' ),
				'type'    => 'text',
				'default' => $this->get_default_api_data( 'client_id' ),
			),
			'api_key'  => array(
	...
	public function get_followers_text( $count ) {
		return sprintf( _n( '%s subscriber', "%s subscribers", $count, 'extra' ), $this->number_format( $count ) );
	}
...
class ET_Tumblr_Social_Followers extends ET_Social_Followers {
	...
	function get_fields() {
		$fields = array(
			'consumer_key'        => array(
				'label'               => __( 'Consumer Key', 'extra' ),
				'type'                => 'password',
				'default'             => $this->get_default_api_data( 'consumer_key' ),
				'authorization_field' => true,
				'description'         => sprintf( __( ' You can find all the info on how to obtain all the required credentials here: %s', 'extra' ), '<a href="http://www.tumblr.com/oauth/apps" target="_blank">http://www.tumblr.com/oauth/apps</a>' ),
			),
			'consumer_secret'     => array(
				'label'               => __( 'Consumer Secret', 'extra' ),
				'type'                => 'password',
				'authorization_field' => true,
				'default'             => $this->get_default_api_data( 'consumer_secret' ),
			),
			'access_token'        => array(
				'label'               => __( 'Access Token', 'extra' ),
				'type'                => 'password',
				'authorization_field' => true,
				'default'             => $this->get_default_api_data( 'access_token' ),
			),
			'access_token_secret' => array(
				'label'               => __( 'Access Token Secret', 'extra' ),
				'type'                => 'password',
				'authorization_field' => true,
	...
	function get_fields() {
		$fields = array(
			'api_key'      => array(
				...
				'default'             => $this->get_default_api_data( 'api_key' ),
				'authorization_field' => true,
				'description'         => sprintf( __( 'You can find all the info on how to obtain all the needed API credentials here: %s', 'extra' ), '<a href="https://dev.twitter.com/oauth/overview/application-owner-access-tokens" target="_blank">https://dev.twitter.com/oauth/overview/application-owner-access-tokens</a>' ),
			),
			'api_secret'   => array(
				'label'               => __( 'Consumer Secret', 'extra' ),
				'type'                => 'password',
