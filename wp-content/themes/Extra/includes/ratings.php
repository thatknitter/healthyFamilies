...
function extra_new_rating() {
	if ( ! wp_verify_nonce( $_POST['rating_nonce'], 'extra_rating_nonce' ) ) {
		die( -1 );
	}

	$post_id = absint( sanitize_text_field( $_POST['post_id'] ) );
	$rating = floatval( sanitize_text_field( $_POST['rating'] ) );
...
function extra_exclude_ratings_from_comments_list( $clauses ) {
	global $wpdb;
	...
	$clauses['where'] .= $wpdb->prepare( ' AND comment_type <> "%s"', EXTRA_RATING_COMMENT_TYPE );

	return $clauses;
}

add_filter( 'comments_clauses', 'extra_exclude_ratings_from_comments_list' );
