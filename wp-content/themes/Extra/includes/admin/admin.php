...
class Extra_Post_Review_Meta_Box extends ET_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'post-review-box', __( 'Review Box Contents', 'extra' ), array(
			'post_type' => 'post',
		) );
	}
	...
	function display( $post ) {
		...
		$title['value'] = !empty( $title['value'] ) ? $title['value'] : __( 'Review', 'extra' );
		...
		$summary_title['value'] = !empty( $summary_title['value'] ) ? $summary_title['value'] : __( 'Summary', 'extra' );
...
class Extra_Gallery_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'gallery-post-format', __( 'Gallery Format Options', 'extra' ), 'gallery' );
	}
	...
	function fields() {
		$this->fields = array(
			...
			'_gallery_format_autoplay'       => array(
				'title'                   => __( 'Autoplay Speed', 'extra' ),
				'description'             => __( 'The speed, in seconds, in which the slider will auto rotate to the next slide. Leave empty to disable autoplay.', 'extra' ),
				'value_sanitize_function' => 'absint',
			),
		);
	}
...
class Extra_Audio_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'audio-post-format', __( 'Audio Format Options', 'extra' ), 'audio' );
	}
	...
	function fields() {
		$this->fields = array(
			'_audio_format_file_url'         => array(
				'title' => __( 'Audio File URL', 'extra' ),
			),
			'_audio_format_background_color' => array(
				'title' => __( 'Background Color', 'extra' ),
			),
			'_audio_format_title'            => array(
				'title' => __( 'Audio File Title', 'extra' ),
			),
			'_audio_format_sub_title'        => array(
				'title' => __( 'Audio File Subtitle', 'extra' ),
			),
		);
	}
...
class Extra_Quote_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'quote-post-format', __( 'Quote Format Options', 'extra' ), 'quote' );
	}

	function fields() {
		$this->fields = array(
			'_quote_format_quote'             => array(
				'title' => __( 'Quote Text', 'extra' ),
			),
			'_quote_format_quote_attribution' => array(
				'title' => __( 'Quote Attribution', 'extra' ),
			),
		);
	}
...
class Extra_Link_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'link-post-format', __( 'Link Format Options', 'extra' ), 'link' );
	}

	function fields() {
		$this->fields = array(
			'_link_format_link_title' => array(
				'title' => __( 'Link Title', 'extra' ),
			),
			'_link_format_link_url'   => array(
				'title' => __( 'Link URL', 'extra' ),
			),
		);
	}
...
class Extra_Video_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'video-post-format', __( 'Video Format Options', 'extra' ), 'video' );
	}
	...
	public function display( $post ) {
		...
		<?php
		printf(
			__( 'The URL of the video from a <a href="%s" target="_blank">Supported oEmbed Video Provider</a>, or URL of any video. Click "Add Video URL" to add more file types. (.mov, .mp4, .webm, .ogv, etc)', 'extra' ),
...
class Extra_Map_Post_Format_Meta_Box extends Extra_Post_Format_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'map-post-format', __( 'Map Format Options', 'extra' ), 'map' );
	}
...
class Extra_Authors_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'authors-page-template', __( 'Authors Page Template Options', 'extra' ), 'page-template-authors.php' );
	}
...
class Extra_Blog_Feed_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'blog-feed-page-template', __( 'Blog Feed Page Template Options', 'extra' ), 'page-template-blog-feed.php' );
	}
	...
	function category_field_renderer( $field ) {
		...
		foreach ( $cats_array as $categs ) {
			...
			$output .= sprintf( '<li><label><input type="checkbox" name="%1$s" value="%2$s" %3$s> %4$s</label></li>',
				...
				in_array( $categs->cat_ID, (array) $field['value'] ) ? ' checked="checked"' : '',
	...
	function fields() {
		$this->fields = array(
			'_blog_feed_page_display_style'       => array(
				'title'       => __( 'Display Style', 'extra' ),
				'type'        => 'select',
				'default'     => strtolower( et_get_option( 'archive_list_style', 'standard' ) ),
				'options'     => array(
					'standard' => __( 'Standard', 'extra' ),
					'masonry'  => __( 'Masonry', 'extra' ),
				),
				'description' => __( 'Display method', 'extra' ),
			),
			'_blog_feed_page_categories'          => array(
				'title'                   => __( 'Categories', 'extra' ),
				'type'                    => 'custom',
			...
			'_blog_feed_page_show_author'         => array(
				'title'       => __( 'Show Author', 'extra' ),
				'type'        => 'select',
				'options'     => array(
					'1' => __( 'Show Author', 'extra' ),
					'0' => __( "Don't Show Author", 'extra' ),
				),
				'description' => __( "Turn the display of each post's author on or off.", 'extra' ),
			),
			'_blog_feed_page_show_categories'     => array(
				'title'       => __( 'Show Categories', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => __( 'Show Categories', 'extra' ),
					'0' => __( "Don't Show Categories", 'extra' ),
				),
				'description' => __( "Turn the display of each post's categories on or off.", 'extra' ),
			),
			'_blog_feed_page_show_featured_image' => array(
				'title'       => __( 'Show Featured Image', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => __( 'Show Featured Image', 'extra' ),
					'0' => __( "Don't Show Featured Image", 'extra' ),
				),
				'description' => __( "Turn the display of each post's featured image on or off.", 'extra' ),
			),
			'_blog_feed_page_show_ratings'        => array(
				'title'       => __( 'Show Ratings', 'extra' ),
				'type'        => 'select',
				'default'     => '1',
				'options'     => array(
					'1' => __( 'Show Ratings', 'extra' ),
					'0' => __( "Don't Show Ratings", 'extra' ),
				),
				'description' => __( "Turn the display of each post's rating stars on or off.", 'extra' ),
			),
			'_blog_feed_page_content_length'      => array(
				'title'       => __( 'Content', 'extra' ),
				'type'        => 'select',
				'options'     => array(
...
class Extra_Sitemap_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'sitemap-page-template', __( 'Sitemap Page Template Options', 'extra' ), 'page-template-sitemap.php' );
	}
	...
	function fields() {
		$this->fields = array(
			...
			'_sitemap_page_pages_exclude'      => array(
				'title'       => __( 'Exclude', 'extra' ),
				'description' => __( 'Page IDs, separated by commas.', 'extra' ),
				'class'       => 'medium',
			),
			'_sitemap_page_categories_count'   => array(
				'title'       => __( 'Show Posts Count', 'extra' ),
				'type'        => 'select',
				'options'     => array(
					'0' => __( 'No', 'extra' ),
					'1' => __( 'Yes', 'extra' ),
				),
				'description' => __( 'Display post count', 'extra' ),
			),
			'_sitemap_page_authors_include'    => array(
				'title'                   => __( 'Authors', 'extra' ),
				'renderer'                => array(
					$this,
					'authors_field_renderer',
				),
				'description'             => __( 'Authors to display', 'extra' ),
				'value_type'              => 'array',
				'value_sanitize_function' => 'absint',
				'default'                 => 'all',//??
			),
			'_sitemap_page_archives_limit'     => array(
				'title'       => __( 'Limit', 'extra' ),
				'description' => __( 'Amount of archive months to display.', 'extra' ),
				'class'       => 'thin',
				'default'     => 12,
			),
			'_sitemap_page_archives_count'     => array(
				'title'       => __( 'Show Post Count', 'extra' ),
				'type'        => 'select',
				'options'     => array(
					'0' => __( 'No', 'extra' ),
					'1' => __( 'Yes', 'extra' ),
				),
				'description' => __( 'Whether to show the post count.', 'extra' ),
			),
			'_sitemap_page_recent_posts_limit' => array(
				'title'       => __( 'Limit', 'extra' ),
				'description' => __( 'Amount of recent posts to display.', 'extra' ),
				'class'       => 'thin',
				'default'     => 10,
			),
	...
	function display( $post ) {
		$sections = array(
			'pages'        => __( 'Pages', 'extra' ),
			'categories'   => __( 'Categories', 'extra' ),
			'tags'         => __( 'Tags', 'extra' ),
			'recent_posts' => __( 'Recent Posts', 'extra' ),
			'archives'     => __( 'Archives', 'extra' ),
			'authors'      => __( 'Authors', 'extra' ),
		);
...
class Extra_Portfolio_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'portfolio-page-template', __( 'Portfolio Page Template Options', 'extra' ), 'page-template-portfolio.php' );
	}
...
class Extra_Contact_Page_Template_Meta_Box extends Extra_Page_Template_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'contact-page-template', __( 'Contact Page Template Options', 'extra' ), 'page-template-contact.php' );
	}
...
class Extra_Layout_Home_Selector_Meta_Box extends ET_Meta_Box {
	...
	public function __construct(){
		parent::__construct( 'layout-home', __( 'Layout Usage', 'extra' ), array(
	...
	function fields() {
		$this->fields = array(
			EXTRA_HOME_LAYOUT_META_KEY    => array(
				'title' => __( 'Use this layout as the home layout?', 'extra' ),
			),
			EXTRA_DEFAULT_LAYOUT_META_KEY => array(
				'title'       => __( 'Use this layout as the default layout?', 'extra' ),
				'description' => __( 'Use as default for any category that does not have a specific layout set.', 'extra' ),
			),
		);
	}
...
class Extra_Project_Details_Meta_Box extends ET_Meta_Box {
	...
	public function __construct(){
		parent::__construct( 'project-details', __( 'Project Details', 'extra' ), array(
	...
	function fields() {
		$this->fields = array(
			EXTRA_PROJECT_DETAILS_TITLE_META_KEY                  => array(
				'title'   => __( 'Project Details Title', 'extra' ),
				'default' => __( 'Project Details:', 'extra' ),
			),
			EXTRA_PROJECT_DETAILS_PROJECT_URL_META_KEY            => array(
				'title' => __( 'Project URL', 'extra' ),
			),
			EXTRA_PROJECT_DETAILS_PROJECT_URL_TEXT_META_KEY       => array(
				'title'   => __( 'Project URL Text', 'extra' ),
				'default' => __( 'View The Project', 'extra' ),
			),
			EXTRA_PROJECT_DETAILS_PROJECT_URL_NEW_WINDOW_META_KEY => array(
				'title' => __( 'Open Project URL in new window', 'extra' ),
				'type'  => 'checkbox',
			),
			EXTRA_PROJECT_DETAILS_LOCATION_META_KEY               => array(
				'title'   => __( 'Project Details Location', 'extra' ),
				'type'    => 'select',
				'options' => array(
					'sidebar'    => __( 'Sidebar, Project Details box below content', 'extra' ),
					'single_col' => __( 'No sidebar, single column view with Project Details box below content', 'extra' ),
					'split'      => __( 'Project Details box as sidebar', 'extra' ),
				),
			),
			EXTRA_PROJECT_DETAILS_SHORT_DESC_META_KEY             => array(
				'title' => __( 'Short description', 'extra' ),
				'type'  => 'textarea',
			),
		);
	}
...
class Extra_Project_Gallery_Meta_Box extends ET_Meta_Box {
	...
	public function __construct() {
		parent::__construct( 'project-gallery', __( 'Project Gallery', 'extra' ), array(
			'post_type' => EXTRA_PROJECT_POST_TYPE,
		) );
	}
...
class Extra_Page_Post_Settings_Meta_Box extends ET_Meta_Box {
	...
	public function __construct(){
		parent::__construct( 'extra-page-post-settings', __( 'Extra Settings', 'extra' ), array(
			'post_type' => array(
				'post',
				'page',
	...
	function fields() {
		...
		$this->fields = array(
			'_extra_sidebar_location' => array(
				'title'      => __( 'Sidebar location', 'extra' ),
				'type'       => 'select',
				'options'    => array(
					''      => sprintf( __( 'Default (%s)', 'extra' ), ucwords( $global_sidebar_location ) ),
					'right' => __( 'Right', 'extra' ),
					'left'  => __( 'Left', 'extra' ),
					'none'  => __( 'No Sidebar', 'extra' ),
				),
				'attributes' => array(
		...
		$this->fields['_extra_sidebar'] = array(
			'title'   => __( 'Choose Sidebar/Widget Area', 'extra' ),
			'type'    => 'select',
			'options' => array(),
		);
		...
		if ( extra_check_feature_availability_in_post_type( $post->post_type, 'hide_title_meta_in_single' ) ) {
			$this->fields['_post_extra_title_meta_hide_single'] = array(
				'title'                   => __( 'Hide Title &amp; Meta on Post', 'extra' ),
		...
		if ( extra_check_feature_availability_in_post_type( $post->post_type, 'hide_featured_image_in_single' ) ) {
			$this->fields['_post_extra_featured_image_hide_single'] = array(
				'title'                   => __( 'Hide Featured Image on Post', 'extra' ),
				'value_sanitize_function' => 'wp_validate_boolean',
				'type'                    => 'checkbox',
			);
		}
		...
		if ( et_get_post_meta_setting( 'all', 'rating_stars' ) && in_array( $post->post_type, extra_get_rating_post_types() ) ) {
			$this->fields['_post_extra_rating_hide'] = array(
				'title'                   => __( 'Hide Post Rating', 'extra' ),
				'value_sanitize_function' => 'wp_validate_boolean',
				'type'                    => 'checkbox',
			);
		}
...
function extra_admin_scripts( $hook ) {
	...
	if ( 'widgets.php' == $hook ) {
		...
		wp_localize_script( 'extra_widgets_js', 'EXTRA', array(
			'label_img_url'   => __( 'Image Url:', 'extra' ),
			'label_link_url'  => __( 'Link Url:', 'extra' ),
			'label_ad_html'   => __( 'Ad HTML:', 'extra' ),
			'label_new_line'  => __( 'Start on New Line', 'extra' ),
			'label_or'        => __( 'OR', 'extra' ),
			'label_ad_number' => __( 'Ad #', 'extra' ),
	...
	if ( 'options-reading.php' == $hook ) {
		...
		if ( $layouts_query->posts ) {
			...
			wp_localize_script( 'extra_admin_options_reading_js', 'EXTRA', array(
				...
				'extra_theme_layout_link' => sprintf( __( 'An <a href="%s">Extra Theme Layout</a> (select below)', 'extra' ), 'edit.php?post_type=' . EXTRA_LAYOUT_POST_TYPE ),
			) );
		}
	...
	if ( in_array( $hook, array( 'post.php', 'post-new.php' ) ) ) {
		...
		if ( 'post' == $typenow || EXTRA_PROJECT_POST_TYPE == $typenow ) {
			...
			wp_localize_script( 'extra_admin_posts_js', 'EXTRA', array(
				'label_breakdown_title'  => __( 'Title:', 'extra' ),
				'label_breakdown_rating' => __( 'Rating (%):', 'extra' ),
				'label_breakdown_number' => __( 'Breakdown #', 'extra' ),
			) );
		}
		...
		if ( EXTRA_LAYOUT_POST_TYPE == $typenow ) {
			...
			wp_localize_script( 'extra_admin_post_layout_js', 'EXTRA', array(
				'category_description' => __( 'Use this layout on the categories selected above.', 'extra' ),
			) );
		}
