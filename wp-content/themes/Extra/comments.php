...
<?php endif; ?>
	<?php if ( have_comments() ) : ?>
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through? ?>
			<div class="comment_navigation_top clearfix">
				<div class="nav-previous"><?php previous_comments_link( __( '<span class="meta-nav">&larr;</span> Older Comments', 'extra' ) ); ?></div>
				<div class="nav-next"><?php next_comments_link( __( 'Newer Comments <span class="meta-nav">&rarr;</span>', 'extra' ) ); ?></div>
			</div> <!-- .navigation -->
	...
	<?php if ( 'open' == $post->comment_status ) : ?>
		<?php comment_form( array('label_submit' => esc_attr__( 'Post Comment', 'extra' ), 'comment_field' => '<p class="comment-form-comment"><label for="comment">' . _x( 'COMMENT', 'noun', 'extra' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>', 'title_reply' => '<span>' . esc_attr__( 'Leave a reply', 'extra' ) . '</span>', 'title_reply_to' => esc_attr__( 'Leave a Reply to %s', 'extra' ) ) ); ?>
	<?php else: ?>
