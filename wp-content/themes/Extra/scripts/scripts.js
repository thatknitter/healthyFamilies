	...
	$(document).ready(function () {
		...
		function paginated_transition_page($this_paginated, to_page) {
			...
			$paginated_pages.filter('.paginated_page_' + to_page).addClass('active').fadeIn('fast', function () {
				...
				$('body').animate({
					scrollTop: ($this_paginated.offset().top - $menu_offset - et_calclate_admin_bar_height() - 20)
				}, {
					duration: 500,
				});
			});
				...
				click: function (rating) {
					$.ajax({
						...
						data: {
							...
							rating_nonce: EXTRA.rating_nonce,
							post_id: $('#post_id').val(),
							rating: rating
						},
						success: function () {
		...
		function et_slider_setup($the_slider) {
			$the_slider.each(function () {
				$the_slider.et_pb_simple_slider({
		...
		if ($featured_posts_slider_module.length) {
			$featured_posts_slider_module.each(function () {
				var $the_slider = $(this),
					autoplay_speed = $the_slider.data('autoplay'),
					autoplay = typeof autoplay_speed !== 'undefined',
					default_speed = 7;

				if (autoplay) {
					if ('' === autoplay_speed) {
						autoplay_speed = default_speed;
					}
				...
				$the_slider.et_pb_simple_slider({
					...
					slideshow: autoplay,
					slideshow_speed: autoplay ? autoplay_speed : default_speed * 1000,
					show_progress_bar: false,
					tabs_animation: false,
					use_carousel: false
				});
