...
(function ($, top_$, EXTRA) {
	...
	function et_print_font_style(styles, important) {
		...
		if ('undefined' !== typeof styles && false !== styles) {
			...
			if (-1 !== styles_array.indexOf('bold')) {
				font_styles = font_styles + "font-weight: bold" + important + "; ";
			} else {
	...
	value_bind_callbacks = {
		...
		extra_et_print_font_style_css_value: function (setting_name, property, unformatted_value) {
			var formatted_value;

			if ('undefined' === typeof unformatted_value) {
				formatted_value = '';
			} else {
					...
					value.bind(function (to) {
						$.each(setting_options.value_bind.property_selectors, function (property, property_selectors) {
							...
							if ('function' === typeof value_bind_callbacks[setting_options.value_bind.value_format_callback]) {
								...
								formatted_value = value_bind_callback(setting_name, property, unformatted_value);
