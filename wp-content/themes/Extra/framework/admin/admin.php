...
class ET_Meta_Box {
	...
	function render_field( $field ) {
		...
		$field['value'] = !empty( $field['value'] ) ? $field['value'] : $default;
...
function et_add_widget_area(){
	...
	$number = $et_widget_areas ? intval( $et_widget_areas['number'] ) + 1 : 1;

	$et_widget_areas['areas']['et_widget_area_' . $number] = sanitize_text_field( $_POST['et_widget_area_name'] );
	$et_widget_areas['number'] = $number;

	$et_widget_areas = update_option( 'et_widget_areas', $et_widget_areas );

	printf( __( '<strong>%1$s</strong> widget area has been created. You can create more areas, once you finish, simply refresh the page to use all the new areas.', $shortname ),
...
add_action( 'wp_ajax_et_add_widget_area', 'et_add_widget_area' );

function et_remove_widget_area(){
	if ( ! wp_verify_nonce( $_POST['et_load_nonce'], 'et_load_nonce' ) ) die( -1 );

	$et_widget_areas = get_option( 'et_widget_areas' );

	unset( $et_widget_areas['areas'][$_POST['et_widget_area_name']] );
...
add_action( 'wp_ajax_et_remove_widget_area', 'et_remove_widget_area' );
