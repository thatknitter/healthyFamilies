...
/**
 * Create post excert of a given length
 *
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
            <article id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
                ...
                <div class="comment_postinfo">
					...
					printf( __( 'on %1$s at %2$s', $shortname ), get_comment_date(), get_comment_time() );
					?>
                    </span>
                    <?php edit_comment_link( esc_html__( '(Edit)', $shortname ), ' ' ); ?>
                <?php
				$et_comment_reply_link = get_comment_reply_link( array_merge( $args, array(
					'reply_text' => esc_attr__( 'Reply', $shortname ),
					'depth'      => (int) $depth,
				...
				<?php if ( '0' == $comment->comment_approved ) : ?>
					<em class="moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', $shortname ) ?></em>
					<br />
				<?php endif; ?>
				...
				if ( $et_comment_reply_link ) {
					echo '<span class="reply-container">' . $et_comment_reply_link . '</span>';
				}
				?>
 ...
 * @return string
 */
function et_print_font_style( $styles = '', $important = '' ) {
	// Prepare variable
	$font_styles = "";
...
if ( ! empty( $this->description ) ) : ?>
	...
	class ET_Color_Alpha_Control extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array() ) {
			$this->statuses = array( '' => __( 'Default', 'extra' ) );
		...
		public function content_template() {
			...
			<label>
				...
				<div class="customize-control-content">
					<input class="color-picker-hex" data-alpha="true" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'extra' ); ?>" {{ defaultValue }} />
				</div>
			</label>
			<?php
		}
...
function et_paginate_links( $args = '' ) {
	$defaults = array(
		...
		'show_all'           => false,
		'prev_next'          => true,
		'prev_text'          => __( '&laquo; Previous', 'extra' ),
		'next_text'          => __( 'Next &raquo;', 'extra' ),
		'beg_size'           => 1,
		'end_size'           => 1,
		'mid_size'           => 2,
		'type'               => 'plain',
	...
	for ( $n = 1; $n <= $args['total']; $n++ ) :
		if ( $n == $args['current'] ) :
			...
			elseif ( $dots && !$args['show_all'] ) :
				$html = '<span class="page-numbers dots">' . __( '&hellip;', 'extra' ) . '</span>';
