...
function et_add_simple_buttons(){
	...
	for ($i = 0; $i <= (count( $buttons ) - 1); $i++) {
		$output .= "edButtons[edButtons.length] = new edButton('extra_{$buttons[$i]['name']}'
...
function et_twitter($atts, $content = null){
	...
	$output = "<script type='text/javascript' src='" . esc_url( "http://twittercounter.com/embed/{$args['name']}/ffffff/111111" ) . "'></script>";
	return $output;
}
...
function et_feedburner($atts, $content = null){
	...
	$output = "<a href='" . esc_url( "http://feeds.feedburner.com/{$args['name']}" ) . "'><img src='" . esc_url( "http://feeds.feedburner.com/~fc/{$args['name']}?bg=99CCFF&amp;fg=444444&amp;anim=0" ) . "' height='26' width='88' style='border:0' alt='' />
</a>";
	return $output;
}
...
function et_box($atts, $content = null) {
	...
	$output = "<div{$args['id']} class='et-box{$args['class']} et-{$args['type']}'>
...
function et_tooltip($atts, $content = null) {
	...
	$output = "<span{$args['id']} class='et-tooltip{$args['class']}'>{$content}<span class='et-tooltip-box'>{$args['text']}<span class='et-tooltip-arrow'></span></span></span>";
...
function et_learnmore($atts, $content = null) {
	...
	$divClass = ($args['state'] == 'close') ? 'et-learn-more' : 'et-learn-more et-open';
	$hClass = ($args['state'] == 'close') ? 'heading-more' : 'heading-more open';
	$divClass .= ' clearfix';

	$output = "<div{$args['id']} class='{$divClass}{$args['class']}'>
					<h3 class='{$hClass}'>" . esc_html( $args['caption'] ) . "<span class='et_learnmore_arrow'><span></span></span></h3>
...
function et_button($atts, $content = null) {
	...
	$output = '';
	$target = ($args['newwindow'] == 'yes') ? ' target="_blank"' : '';
	...
	if ($args['type'] == 'small')
		$output .= "<a{$args['id']} href='" . esc_url( $args['link'] ) . "' class='" . esc_attr( "small-button small{$args['color']}{$args['class']}" ) . "'{$target}>{$content}</a>";

	if ($args['type'] == 'big')
		$output .= "<a{$args['id']} href='" . esc_url( $args['link'] ) . "' class='" . esc_attr( "big-button big{$args['color']}{$args['class']}" ) . "'{$target}>{$content}</a>";

	if ($args['type'] == 'icon')
		$output .= "<a{$args['id']} href='" . esc_url( $args['link'] ) . "' class='" . esc_attr( "icon-button {$args['icon']}-icon{$args['class']}" ) . "'{$target}>{$content}<span class='et-icon'></span></a>";

	if ( $args['br'] == 'yes' ) $output .= '<br class="clear"/>';
...
function et_slide($atts, $content = null) {
	...
	$output = "<div{$args['id']} class='et-slide{$args['class']}'>
					{$content}
				</div>";
...
function et_tabs($atts, $content = null) {
	...
	$args['auto'] = ( $args['auto'] == 'no' ) ? 'false' : 'true';
	...
	$args['class'] .= " et_sliderfx_{$args['fx']}" . " et_sliderauto_{$args['auto']}" . " et_sliderauto_speed_{$args['autospeed']}";

	if ($args['slidertype'] == 'top tabs') {
		$args['class'] .= ' et_slidertype_top_tabs';
		$output = "
			<div class='" . esc_attr( "et-tabs-container{$args['class']}" ) ."'{$args['id']}>
				{$content}
			</div> <!-- .et-tabs-container -->";
	} elseif ($args['slidertype'] == 'left tabs') {
		$args['class'] .= ' et_slidertype_left_tabs clearfix';
		$output = "
			<div class='" . esc_attr( "tabs-left{$args['class']}" ) . "'{$args['id']}>
				<div class='et_left_tabs_bg'></div>
				{$content}
			</div> <!-- .tabs-left -->";
	} elseif ($args['slidertype'] == 'simple') {
		$args['class'] .= ' et_slidertype_simple';
		$output = "
		<div class='" . esc_attr( "et-simple-slider{$args['class']}" ) . "'{$args['id']}>
		...
		";
	} elseif ($args['slidertype'] == 'images') {
		$args['class'] .= ' et_slidertype_images';
		$output = "
		<div class='" . esc_attr( "et-image-slider{$args['class']}" ) . "'{$args['id']}>
...
function et_imagetabcontainer($atts, $content = null) {
	...
	$output = "
		<div{$args['id']} class='controllers-wrapper{$args['class']}'>
...
function et_tabcontent($atts, $content = null) {
	...
	$output = "
		<div{$args['id']} class='et-tabs-content{$args['class']}'>
...
function et_tab($atts, $content = null) {
	...
	$output = "
		<div{$args['id']} class='et_slidecontent{$args['class']}'>
			{$content}
		</div>";
...
function et_imagetab($atts, $content = null) {
	...
	$target = '' != $args['newwindow'] ? ' target="_blank"' : '';
	$content = et_content_helper( $content );
	...
	$output = "
		<div{$args['id']} class='et_slidecontent et-image-slide{$args['class']}'>" . ( '' != $args['link'] ? "<a href='" . esc_url( $args['link'] ) . "'{$target}>" : '' ) . et_new_thumb_resize( $content, $args['width'], $args['height'], $args['alt'] ) . "<span class='et-image-overlay'> </span>" . ( '' != $args['link'] ? '</a>' : '' ) . "</div>";
...
function et_author($atts, $content = null) {
	...
	$output = "
		<div{$args['id']} class='author-shortcodes{$args['class']}'>
...
function et_author_image($atts, $content = null) {
	...
	$src = ($args['timthumb'] == 'on') ? ( et_new_thumb_resize( $content, 60, 60, '', $forstyle = true ) ) : $content;
...
function et_custom_list($atts, $content = null) {
	...
	$args['type'] = ( $args['type'] <> 'checkmark' ) ? ' etlist-' . $args['type'] : '';

	$output = "
		<div class='et-custom-list{$args['type']}'>
			{$content}
		</div> <!-- .et-custom-list -->";
...
function et_pricing($atts, $content = null) {
	...
	$separator_sign = ( strpos( $args['price'], '.' ) !== false ) ? '.' : ',';
	$price_array = explode( $separator_sign, $args['price'] );
	$link_target = ( $args['window'] == 'new' ) ? ' target="_blank"' : '';
	$args['type'] = ( $args['type'] == 'big' ) ? ' pricing-big' : '';

	$output = "
		<div class='" . esc_attr( 'pricing-table' . $args['type'] ) . "'>
			<div class='pricing-table-wrap'>
				<div class='pricing-heading'>
					<h2 class='pricing-title'>" . esc_html( $args['title'] ) . "</h2>
				...
				<div class='pricing-content-bottom'>
					<span class='et-price'><span class='dollar-sign'>" . esc_html( $args['currency'] ) . "</span>" . esc_html( $price_array[0] ) .
						"<sup>" . esc_html( $price_array[1] ) . "</sup></span>
				</div> <!-- end .pricing-content-bottom -->

				<a href='" . esc_url( $args['url'] ) . "' class='icon-button'{$link_target}>" . esc_html( $args['moretext'] ) . "</a>
...
function et_pricing_feature($atts, $content = null) {
	...
	$content = et_content_helper( $content );
	$class = ( $args['checkmark'] == 'x' ) ? ' class="x-mark"' : '';
...
function et_dropcap($atts, $content = null) {
	...
	$output = "<span{$args['id']} class='et-dropcap{$args['class']}'{$args['style']}>{$content}</span>";
...
function et_quote($atts, $content = null) {
	...
	$args['id'] = ($args['id'] <> '') ? " id='" . esc_attr( $args['id'] ) . "'" : '';
	$args['class'] = ($args['class'] <> '') ? esc_attr( ' ' . $args['class'] ) : '';
	if ( $args['type'] == 'center' ) $args['class'] = ' quote-center';

	$output = "
		<div{$args['id']} class='et_quote{$args['class']}'{$args['style']}>
			<div class='et_right_quote'>
				{$content}
				" . ( 'center' == $args['type'] ? "<span class='et_quote_additional_sign'></span>" : '' ) . "
			</div>
...
function et_columns($atts, $content = null, $name='') {
	...
	$output = "<div{$args['id']} class='" . esc_attr( $name . $args['class'] ) . "'>
					{$content}
				</div>";
	if($pos !== false)
