			...
			<div class="et_pb_extra_column_main">
				...
				if ( have_posts() ) :
					while ( have_posts() ) : the_post(); ?>
						<?php $post_category_color = extra_get_post_category_color(); ?>
						<article id="post-<?php the_ID(); ?>" <?php post_class( 'module single-post-module' ); ?>>
							...
							<?php if ( !extra_is_builder_built() ) { ?>
								<div class="post-content">
									...
									<?php
										wp_link_pages( array(
											'before' => '<div class="page-links">' . __( 'Pages:', 'extra' ),
											'after'  => '</div>',
										) );
									?>
								</div>
									...
									<div class="review-content">
										<div class="review-summary clearfix">
											<div class="review-summary-score-box" style="background-color:<?php echo esc_attr( $post_category_color ); ?>">
												<h4><?php printf( __( '%d%%', 'extra' ), absint( $review['score'] ) ); ?></h4>
											</div>
											...
											<div class="review-breakdown">
												...
												<div class="score-bar-bg">
													<span class="score-bar" style="background-color:<?php echo esc_attr( $post_category_color ); ?>; width:<?php printf( '%d%%', max( 4, absint( $breakdown['rating'] ) ) );  ?>">
														<span class="score-text"><?php printf( __( '%d%%', 'extra' ), absint( $breakdown['rating'] ) ); ?></span>
													</span>
												</div>
											</div>
											<?php } ?>
						...
						<nav class="post-nav">
							<div class="nav-links clearfix">
								<div class="nav-link nav-link-prev">
									<?php previous_post_link( '%link', __( '<span class="button">Previous</span><span class="title">%title</span>', 'extra' ) ); ?>
								</div>
								<div class="nav-link nav-link-next">
									<?php next_post_link( '%link', __( '<span class="button">Next</span><span class="title">%title</span>', 'extra' ) ); ?>
								</div>
							</div>
						</nav>
						<div class="et_extra_other_module author-box">
							...
							<div class="author-box-content clearfix">
								...
								<div class="author-box-description">
									<h4><a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author" title="<?php printf( __( 'View all posts by %s', 'extra' ), get_the_author() ); ?>"><?php echo get_the_author(); ?></a></h4>
				...
				<?php
				if ( comments_open() && 'on' == et_get_option( 'extra_show_postcomments', 'on' ) ) {
