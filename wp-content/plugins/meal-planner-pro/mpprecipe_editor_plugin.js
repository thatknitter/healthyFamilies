...
(function() {
	...
	tinymce.create('tinymce.plugins.EditMPPRecipe', {
		init: function( editor, url ) {
			...
			*/
			...
			editor.addButton( 'mpprecipe', {
				title: 'MealPlannerPro Recipe',
				image: url + '/mpprecipe.png',
				onclick: function() {
					var recipe_id = null;
					if (recipe = editor.dom.select('img.mpprecipe-recipe')[0]) {
						editor.selection.select(recipe);
						recipe_id = /mpprecipe-recipe-([0-9]+)/i.exec(editor.selection.getNode().id);
					}
					var iframe_url = baseurl + '/wp-admin/media-upload.php?post_id=' + ((recipe_id) ? '1-' + recipe_id[1] : mpp_post_id) + '&type=mpprecipe&tab=mpprecipe&TB_iframe=true&width=640&height=523';
		...
		_convert_codes_to_imgs : function(co) {
            return co.replace(/\[mpprecipe-recipe:([0-9]+)\]/g, function(a, b) {
								return '<img id="mpprecipe-recipe-'+b+'" class="mpprecipe-recipe" src="' + plugindir + '/mpprecipe-placeholder.png" alt="" />';
            });
		},

		_convert_imgs_to_codes : function(co) {
			return co.replace(/\<img[^>]*?\sid="mpprecipe-recipe-([0-9]+)[^>]*?\>/g, function(a, b){
                return '[mpprecipe-recipe:'+b+']';
            });
		},

		getInfo : function() {
            return {
                longname : "MealPlannerPro Recipe Plugin",
                author : 'MealPlannerPro, Inc.',
                authorurl : 'http://www.mealplannerpro.com/',
                infourl : 'http://www.mealplannerpro.com/recipe_plugin',
                version : "5.3"
            };
        }
	});

	tinymce.PluginManager.add('mpprecipe', tinymce.plugins.EditMPPRecipe);
